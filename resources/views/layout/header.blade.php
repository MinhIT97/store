<head>
    <header class="lion-head">
        <div class="lion-wrapper">
            <div class="container">

                <div class="lion-nav">
                    <div class="lion-nav__left">
                        <ul>
                            <li class="lion-nav__iteam"><a href="">Nam</a></li>
                            <li class="lion-nav__iteam"><a href="">Nữ</a></li>
                            <li class="lion-nav__iteam"><a href="">Unisex</a></li>
                            <li class="lion-nav__iteam"><a href="">Phụ kiện</a></li>
                        </ul>
                    </div>
                    <div class="lion-logo">
                        <a href="/" class="lion-logo__image"><img src="/images/shoplogo.png" alt=""></a>
                    </div>

                    <div class="lion-nav__right">
                        <ul>
                            <li class="lion-nav__iteam"><a href="">Blog</a></li>
                            <li class="lion-nav__iteam"><a href="">Đăng nhập</a></li>
                            <li class="lion-nav__iteam"><a href="">Giỏ hàng</a></li>
                            <li class="lion-nav__iteam"><a href=""><i class="fas fa-search"></i></a></li>
                        </ul>
                    </div>

                </div>

            </div>
        </div>
    </header>

</head>
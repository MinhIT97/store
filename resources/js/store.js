...
$(document).ready(function () {
    ...
    function imuplCloseEditor() {
        ...
        img.cropEndX =
            ($imuplCropWrapper.width() - parseInt($imuplCropper.css("right"))) /
            imuplEditorRatio;
        img.cropEndY =
            ($imuplCropWrapper.height() -
                parseInt($imuplCropper.css("bottom"))) /
            imuplEditorRatio;
        console.log(img);
        setTimeout(function () {
    ...
    function imuplOpenEditor(offset) {
        ...
        $imuplCropper
            .css("left", img.cropStartX * ratio)
            .css("top", img.cropStartY * ratio);
        $imuplCropper
            .css("right", imgWidth - img.cropEndX * ratio)
            .css("bottom", imgHeight - img.cropEndY * ratio);
    ...
    function imuplAddFile(f) {
        if (
            imuplFilesCurrent >= imuplFilesMax ||
        ...
        var thisElement = $(
            '<div class="imupl-file-item loading" data-imupl-offset="' +
            thisOffset +
            '"><div class="imupl-button-remove"><i class="mdi mdi-delete-forever"></i></div><div class="imupl-button-edit"><i class="fa fa-crop"></i></div><input type="hidden" name="imupl-payload[]" value="" /></div>'
        );
        var reader = new FileReader();
        reader.onload = function (e) {
            var img = new Image();
    ...
    function imuplMoveCropper(e) {
        ...
        if (imuplCropperDragging == 1) {
            ...
            posX = Math.min(
                posX,
                $imuplCropWrapper.width() -
                parseInt($imuplCropper.css("right")) -
                40
            );
            posY = Math.min(
                posY,
